...
function encryptFormText() {
	...
	if(document.form.text.value.length == 0) {
		alert("No text to encrypt!  Please enter or paste plain text in the field above.");
		return;
	}
	encoded = GibberishAES.enc(document.form.text.value, document.form.key.value);
	document.form.text.value = encoded;
...
function decryptFormText() {
	...
	if(document.form.text.value.length == 0) {
		alert("No text to decrypt!  Please enter or paste cipher text in the field above.");
		return;
	}
	
	try {
		var dec = GibberishAES.dec(document.form.text.value, document.form.key.value);
		document.form.text.value = dec;
	} catch (err) {
		alert("Invalid key");
	}
};
...
function load() {
	...
	document.form.text.value = decodeURIComponent(gup("text"));
	document.encryptedCode.text.value = "";
